...
import static com.sequenceiq.flow.core.chain.init.config.FlowChainInitEvent.FLOWCHAIN_INIT_FINALIZE_EVENT;
...
import static com.sequenceiq.flow.core.chain.init.config.FlowChainInitState.FLOWCHAIN_INIT_STATE;
...
@Component
public class FlowChainInitFlowConfig extends AbstractFlowConfiguration<FlowChainInitState, FlowChainInitEvent> implements RetryableFlowConfiguration<FlowChainInitEvent> {
    private static final List<Transition<FlowChainInitState, FlowChainInitEvent>> TRANSITIONS = new Transition.Builder<FlowChainInitState, FlowChainInitEvent>()
            .defaultFailureEvent(FLOWCHAIN_INIT_FAILED_EVENT)

            .from(INIT_STATE)
            .to(FLOWCHAIN_INIT_STATE)
            .event(FLOWCHAIN_INIT_TRIGGER_EVENT)
            .noFailureEvent()

            .from(FLOWCHAIN_INIT_STATE)
            .to(FINAL_STATE)
            .event(FLOWCHAIN_INIT_FINALIZE_EVENT)
            .defaultFailureEvent()
