...
import com.sequenceiq.flow.core.chain.init.flowevents.FlolwChainInitFailedPayload;
...
public class FlowChainInitActions {
    ...
    @Bean(name = "FLOWCHAIN_INIT_STATE")
    public Action<?, ?> initAction() {
        return new AbstractFlowChainInitAction<>(FlowChainInitPayload.class) {
            ...
            protected void doExecute(CommonContext context, FlowChainInitPayload payload, Map<Object, Object> variables) {
                ...
                sendEvent(context, FLOWCHAIN_INIT_FINALIZE_EVENT.event(), payload);
            }
        };
    }
    ...
    public Action<?, ?> failedAction() {
        return new AbstractFlowChainInitAction<>(FlolwChainInitFailedPayload.class) {
            @Override
            protected void doExecute(CommonContext context, FlolwChainInitFailedPayload payload, Map<Object, Object> variables) {
    ...
    private abstract static class AbstractFlowChainInitAction<P extends FlowChainInitPayload>
        ...
        protected Object getFailurePayload(P payload, Optional<CommonContext> flowContext, Exception ex) {
            return new FlolwChainInitFailedPayload(payload.getFlowChainName(), payload.getResourceId(), ex);
        }
    }
}
