...
import static com.sequenceiq.flow.core.chain.finalize.config.FlowChainFinalizeState.FLOWCHAIN_FINALIZE_STATE;
...
@Component
public class FlowChainFinalizeFlowConfig extends AbstractFlowConfiguration<FlowChainFinalizeState, FlowChainFinalizeEvent> implements RetryableFlowConfiguration<FlowChainFinalizeEvent> {
    private static final List<Transition<FlowChainFinalizeState, FlowChainFinalizeEvent>> TRANSITIONS = new Transition.Builder<FlowChainFinalizeState, FlowChainFinalizeEvent>()
            .defaultFailureEvent(FLOWCHAIN_FINALIZE_FAILED_EVENT)

            .from(INIT_STATE)
            .to(FLOWCHAIN_FINALIZE_STATE)
            .event(FLOWCHAIN_FINALIZE_TRIGGER_EVENT)
            .noFailureEvent()

            .from(FLOWCHAIN_FINALIZE_STATE)
            .to(FINAL_STATE)
            .event(FLOWCHAIN_FINALIZE_FINALIZE_EVENT)
            .defaultFailureEvent()
