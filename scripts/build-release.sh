...
./gradlew -Penv=jenkins -b build.gradle build uploadBootArchives :freeipa-client:uploadArchives :cluster-proxy:uploadArchives :common:uploadArchives -Preckon.scope=$SCOPE -Preckon.stage=final --refresh-dependencies --info --stacktrace --parallel -x checkstyleMain -x checkstyleTest -x spotbugsMain -x spotbugsTest
